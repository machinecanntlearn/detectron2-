...
from YOLOv1.model.modeling import DarkNet
from YOLOv1.model.modeling import resnet_1024ch
from _bulid_model import META_ARCH_REGISTRY
...
#
#         self.s = cfg.MODEL.BACKBONE.S
#         self.backbone = None
#         self.conv = None
#         self.pretrain = cfg.MODEL.BACKBONE.PRETRAIN
#         self.cell_out_ch = cfg.MODEL.BACKBONE.OUT_CH
#         self.backbone_name = cfg.MODEL.BACKBONE.NAME
#         if self.backbone_name == 'darknet':
#             self.backbone = DarkNet()
#         elif self.backbone_name == 'resnet':
#             self.backbone = resnet_1024ch(pretrained=self.pretrain)
#
...
#             nn.LeakyReLU(0.1),
#             nn.Dropout(0.5),
#             nn.Linear(4096, self.s * self.s * self.cell_out_ch)
#         )
#
#         self.criterion = yolo_loss(cfg)
#     def post_yolo(self,feature,labels):
#         x = torch.flatten(feature, 1)
#         x = self.fc(x)
#         x = x.view(self.batch_size, self.s ** 2, -1)
#         loss, _, _, conf_loss, class_loss = criterion(x, labels)
#         detloss = {
#             "loss" : loss,
#             "loss_cls": conf_loss,
#             "loss_box_reg": class_loss,
#         }
#         return detloss
#
#     def forward(self, batched_inputs):
#         self.batch_size = batched_inputs.size(0)
#         if not self.training:
#             return self.inference(batched_inputs)
#         assert "instances" in batched_inputs[0]
#         gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
#         detector_losses, _, _ = self._forward_once_(batched_inputs, gt_instances)
#         losses = {}
...
#         _, results, image_sizes = self._forward_once_(batched_inputs, None)
#         processed_results = []
...
#         if self.training:
#             loss = self.post_yolo(features, gt_instances)
#             return loss, {}, images.image_sizes
#         else:
#             results = self.output_process(features)
#             return {}, results, images.image_sizes
#
...
class yolo_loss:
    def __init__(self, cfg):
        ...
        self.s = cfg.MODEL.BACKBONE.S
        self.b = cfg.MODEL.BACKBONE.B
        self.image_size = cfg.INPUT.IMG_SIZE_TRAIN
...
if __name__ == "__main__":
    ...
    # B * 5 + n_classes
    net = META_ARCH_REGISTRY.get("YOLOv1")
    net = YOLOv1(7, 2 * 5 + 20, 'resnet', pretrain=None)
